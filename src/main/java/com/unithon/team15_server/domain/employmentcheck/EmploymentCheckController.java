...
public class EmploymentCheckController {
    ...
    @Operation(summary = "내 시간제취업 정보 전체 조회", description = "- 모든 step에 대한 체크리스트 정보를 전달\n" +
            "- step의 정보와 유의사항 전달\n")
    @ApiResponse(responseCode = "200", description = "내 시간제취업 정보 전체 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = HomeInfoRes.class)))
    @GetMapping("/home")
    ...
    @ApiResponse(responseCode = "200", description = "Tip 정보 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TipInfoRes.class)))
    @GetMapping("/tips")
